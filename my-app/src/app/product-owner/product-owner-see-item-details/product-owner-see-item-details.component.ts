...
import { AllItemInfo } from '../product-owner-item-statistics/product-owner-item-statistics.component';


const MESSAGE_TIMEOUT = 5000; 

@Component({
  selector: 'app-product-owner-see-item-details',
  templateUrl: './product-owner-see-item-details.component.html',
  styleUrls: ['./product-owner-see-item-details.component.scss']
})
export class ProductOwnerSeeItemDetailsComponent {
  name : string | null = "";
  surname : string | null = "";
  role : string | null = "";
  ...
  getItems(){
    this.requestService.getAllItemDeteils(this.productID).subscribe((response: { allItems: AllItems[]}) => {
      console.log(response);
      if (response) {
        this.items.next([]); 
